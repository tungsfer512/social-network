...
class MainController extends \humhub\modules\admin\components\Controller{
	...
	public function MyDataRequest(){
		...
		if(Yii::$app->request->get('dlInactiveAccnts')=='Yes'){
			...
			$dlInactiveAccnts_cmd=Yii::$app->db->createCommand("SELECT user.id as UserID, user.email, user.username FROM user WHERE last_login is null;")->queryAll(); 
			foreach($dlInactiveAccnts_cmd as $dlInactiveAccnts_row){
				...
				$dlInactiveAccntsFile.=$dlInactiveAccnts_row['email'].$MyTabChar.$dlInactiveAccnts_row['username'].$MyTabChar.substr($EachUserGroupNames,0,-4)."\n";
				}
